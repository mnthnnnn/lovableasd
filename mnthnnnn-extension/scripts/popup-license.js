...
(function () {
  ...
  function handleStatus(res) {
    ...
    if (res && res.ok && res.status === 'active') {
      ...
      // ── Welcome message ──
      const welcomeEl    = el('active-welcome');
      const welcomeSubEl = el('active-welcome-sub');
      const firstName    = (res.userName || 'User').split(' ')[0];
      if (welcomeEl)    welcomeEl.textContent    = `Welcome, ${firstName}! 👋`;
      if (welcomeSubEl) welcomeSubEl.textContent = `You're all set. Lovable is unlocked.`;
      showState('active');
